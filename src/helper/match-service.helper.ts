...
export class MatchServiceHelper {
    ...
    static isAutoMatch(recordFetched: { [key: string]: any }, reconciliationQuery: ReconciliationQuery,
        ...
        const matchers = {
            ...
            closeDates: (startDateA: string, startDateB: string,
                         endDateA: string | undefined, endDateB: string | undefined) => {
                const dateStampA = endDateA ? new Date(endDateA).getTime() : new Date(startDateA).getTime();
                const dateStampB = endDateB ? new Date(endDateB).getTime() : new Date(startDateB).getTime();
        ...
        if (reconciliationQuery.type === Entities.PLACE) {
            return checkIfWikidataIdIsExactMatch.every(Boolean) ||
            ...
            return checksNameStartDateEndDatePlaceUriMatchForEvents.every(Boolean) ||
    ...
    private static formatReconciliationQuery(reconciliationQuery: ReconciliationQuery) {
        ...
        const name = conditions.find((condition) =>
            condition.matchType === "name")?.propertyValue as string | undefined;
        const postalCode = conditions.find((condition) =>
            condition.propertyId?.includes("postalCode"))?.propertyValue as string | undefined;
        const addressLocality = conditions.find((condition) =>
            condition.propertyId?.includes("addressLocality"))?.propertyValue as string | undefined;
        const addressRegion = conditions.find((condition) =>
            condition.propertyId?.includes("addressRegion"))?.propertyValue as string | undefined;
        const url = conditions.find((condition) =>
            condition.propertyId?.includes("url"))?.propertyValue as string | undefined;
        ...
        const startDate = conditions.find((condition) =>
            condition.propertyId?.includes("startDate"))?.propertyValue as string | undefined;
        const endDate = conditions.find((condition) =>
            condition.propertyId?.includes("endDate"))?.propertyValue as string | undefined;

        const locationName = conditions.find((condition) =>
            condition.propertyId?.includes("<https://schema.org/location>/<https://schema.org/name>"))
            ?.propertyValue as string | undefined;
        const locationUri = conditions.find((condition) =>
            condition.propertyId?.includes("<https://schema.org/location>/<https://schema.org/sameAs>"))
            ?.propertyValue as string | undefined;
